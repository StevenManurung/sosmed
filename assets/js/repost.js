$(function(){
	$(document).on('click','.repost', function(){
		...
		$.post('http://localhost/sosmed/core/ajax/repost.php', {showPopup:post_id,user_id:user_id}, function(data){
			$('.popupPost').html(data);
	...
	$(document).on('click', '.repost-it', function(){
	    ...
	    $.post('http://localhost/sosmed/core/ajax/repost.php', {repost:post_id,user_id:user_id,comment:comment}, function(){
	    	$('.repost-popup').hide();
	    	$count++;
	    	$counter.text($count);
