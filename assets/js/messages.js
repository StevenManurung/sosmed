$(function(){
	...
	$(document).on('click', '.people-message', function(){
		...
		getMessages = function(){
			$.post('http://localhost/sosmed/core/ajax/messages.php', {showChatMessage:get_id}, function(data){
		...
		$(document).on('click', '.deleteMsg', function(){
			...
			$(document).on('click', '.delete', function(){
				$.post('http://localhost/sosmed/core/ajax/messages.php', {deleteMsg:messageID}, function(data){
					$('.message-del-inner').height('0px');
					getMessages();
				})
			});
